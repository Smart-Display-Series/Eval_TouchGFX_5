...
void STM32TouchController::init()
{
	...
	/* FT5426 */
	if(HAL_I2C_Mem_Read( phi2c, 0x70, 0x02, I2C_MEMADD_SIZE_8BIT, &Buf[0], 5, 10 ) == HAL_OK)
		ctp_driver = FT5426;
	/* ILI2130 */
	else if( HAL_I2C_Mem_Read( phi2c, 0x82, 0x10, I2C_MEMADD_SIZE_8BIT, &Buf[0], 64, 10 ) == HAL_OK )
		ctp_driver = ILI2130;
	...
	switch( touch_type )
	{
		...
		case CTP:
			...
			else if( ctp_driver == ILI2130 )
			{
				...
				if( HAL_I2C_Mem_Read( phi2c, 0x82, 0x10, I2C_MEMADD_SIZE_8BIT, &Buf[0], 32, 10 ) == HAL_OK )
				{
					if( Buf[0] == 0x48 )
					{
					...
					HAL_I2C_Master_Receive( phi2c, 0x83, &Buf[0], 32, 10 );
				}
				else
				{
					test_x ++;
